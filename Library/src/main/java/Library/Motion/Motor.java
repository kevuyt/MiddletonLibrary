...
import Library.Sensors.TouchSensor;

import static Library.Resources.Utils.*;
...
public class Motor {
    ...
    private TouchSensor minLim, maxLim = null;
    private final String name;
    private double min = -1, max = 1;

    public Motor(String name, Direction direction, double pidAdjust) {
    ...
    public Motor(String name, Direction direction) {this(name, direction, 1);}
    public Motor(String name, double pidAdjust) {this(name, FORWARD, pidAdjust);}
    public Motor(String name) {this(name, FORWARD, 1);}
    ...
    public double getClicksPerInch() {
        return (motor.getMotorType().getTicksPerRev() / (wheelDiameter * PI)) * gearRatio;
    }
    ...
    public void setMinLimit(TouchSensor min) {
        minLim = min;
        hasMin = true;
    }
    public void setMaxLimit(TouchSensor max) {
        maxLim = max;
        hasMax = true;
    }
