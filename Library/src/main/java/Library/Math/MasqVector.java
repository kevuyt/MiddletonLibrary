...
public class MasqVector {
    ...
    public MasqVector multiply(double scalar) {
        return new MasqVector(name, getX() * scalar, getY() * scalar);
    }
    public MasqVector displacement(MasqVector v) {
        return new MasqVector(name, v.getX() - getX(), v.getY() - getY());
    }
    public MasqVector projectOnTo(MasqVector v) {
        return v.unitVector().multiply(dotProduct(v.unitVector()));
    }
    public MasqVector add(MasqVector v) {
        return new MasqVector(name, getX() + v.getX(), getY() + v.getY());
    }
    public double dotProduct(MasqVector v) {
        return (this.getX() * v.getX()) + (this.getY() * v.getY());
    }
    public double angleTo(MasqVector v) {
        return toDegrees(atan2(getY(), getX()) - atan2(v.getY(), v.getX()));
    }
    public double distanceToVector(MasqVector v) {
